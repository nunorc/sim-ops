...
export default {
	...
	methods: {
		updateData(data) {
			...
			this.chartPower.series[0].data.push(this.state.pts_net_power.toFixed(2));
	...
	<div class="row" v-if="renderComponent && !compact">
		...
		<div class="col-xl-2 col-lg-2">
			<card class="mb-3">
				...
				<card-body class="p-2 mx-2">
					<span v-if="state" class="badge rounded-0 app-w-100 text-uppercase" :class="{ 'bg-success': state.pts_sol_array[0] === 'nominal', 'bg-danger': state.pts_sol_array[0] !== 'nominal' }">{{ state.pts_sol_array[0] }}</span>
					<span v-else>_</span>
				</card-body>
			</card>
		</div>
		<div class="col-xl-2 col-lg-2">
			<card class="mb-3">
				...
				<card-body class="p-2 mx-2">
					<span v-if="state" class="badge rounded-0 app-w-100 text-uppercase" :class="{ 'bg-success': state.pts_sol_array[1] === 'nominal', 'bg-danger': state.pts_sol_array[1] !== 'nominal' }">{{ state.pts_sol_array[1] }}</span>
					<span v-else>_</span>
				</card-body>
			</card>
		</div>
	...
	<div class="row" v-if="renderComponent && !compact">
		...
		<div class="col-xl-2 col-lg-2">
			<card class="mb-3">
				...
				<card-body>
					<div class="row align-items-center" v-if="state">
						<h3 class="mb-0 text-center">{{ state.pts_net_power.toFixed(2) }}</h3>
						<div class="mt-1">
